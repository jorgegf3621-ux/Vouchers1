  ...
  const load = useCallback(async () => {
    ...
    try {
      const [sessRes, progRes] = await Promise.all([
        supabase.from('voucher_sessions').select('voucher_number,case_specialist,status,next_call_date').order('next_call_date').limit(10000),
        supabase.from('voucher_progress').select('voucher_number,completed,new_call_date,completed_at').limit(10000),
      ])
