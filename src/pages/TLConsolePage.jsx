...
export default function TLConsolePage() {
  ...
  const filteredViewVouchers = useMemo(() => voucherFilter === 'All' ? viewVouchersSessions : viewVouchersSessions.filter(s => s.status === voucherFilter), [viewVouchersSessions, voucherFilter])
          ...
          {filterSpec ? (
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 10 }}>
                {specStats.map(({ spec, total, overdue, done, left, byStatus }) => { const pct = overdue ? Math.round(done / overdue * 100) : 100; const color = pct >= 80 ? 'var(--green)' : pct >= 50 ? 'var(--amber)' : 'var(--red)'; const isViewing = viewVouchers === spec; return (
              ...
              {viewVouchers && (
                <Card style={{ marginTop: 14 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}><div><div style={{ fontSize: 13, fontWeight: 700 }}>📋 {viewVouchers} — Voucher Breakdown</div><div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>Click a voucher number to open it</div></div><Btn variant="default" size="sm" onClick={e => { e.stopPropagation(); setViewVouchers(null) }}>✕ Close</Btn></div>
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 12 }}>{['All', 'Contacted', 'Processing', 'Pending', 'Filed'].map(st => { const isActive = voucherFilter === st; const count = st === 'All' ? viewVouchersSessions.length : viewVouchersSessions.filter(s => s.status === st).length; return <button key={st} onClick={() => setVoucherFilter(st)} style={{ padding: '4px 12px', fontSize: 11, borderRadius: 20, border: '1px solid var(--border2)', background: isActive ? 'var(--text)' : 'var(--bg3)', color: isActive ? 'var(--bg)' : 'var(--text2)', cursor: 'pointer', fontWeight: isActive ? 600 : 400 }}>{st} ({count})</button> })}</div>
